...
import EndGame.config.CarSellStatus;
...
class CarRepositoryTest {
    ...
    public void createCarList() {
        for (int i = 1; i <= 10; i++) {
            Car car = new Car();
            car.setCarName("테스트용 자동차 No " + i);
            car.setType("대형");
            car.setBrand("BMW");
            car.setColor("red");
            car.setPrice(10000 + i);
            car.setManufacture(2010 + i);
            car.setCarDetail("테스트 상품 상세 설명");
            car.setStockNumber(1);
            car.setCarSellStatus(CarSellStatus.SELL);
            car.setRegTime(LocalDateTime.now());
            car.setUpdateTime(LocalDateTime.now());
    ...
    public void findByCarNameTest() {
        this.createCarList();
        List<Car> carList = carRepository.findByCarName("테스트 상품1");
        for (Car car : carList) {
    ...
    @Test
    @DisplayName("금액 LessThan 조회 테스트")
